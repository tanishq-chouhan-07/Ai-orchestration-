...
import { detectAnomalies, getTrends } from "@/services/analytics";
...
export async function GET(request: Request) {
  ...
  const days = Math.min(Math.max(Number(daysParam), 3), 90);
  ...
  try {
    ...
    const trends = getTrends(
      executions as Array<{ status?: string; startedAt?: string; stoppedAt?: string }>,
      days
    );
