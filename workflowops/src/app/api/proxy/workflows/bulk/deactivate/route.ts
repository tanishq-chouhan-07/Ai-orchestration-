...
export async function POST(request: Request) {
  try {
    ...
    const results = await Promise.allSettled(
      payload.workflowIds.map((id) => deactivateWorkflow(instance.url, apiKey, id))
    );
    ...
    await writeAuditLog({
      userId,
      action: "workflows.bulk_deactivate",
      resource: "bulk",
    ...
    return NextResponse.json({ error: "Bulk deactivate failed" }, { status: 500 });
  }
}
